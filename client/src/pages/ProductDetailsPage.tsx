  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="aspect-square bg-gradient-to-br from-slate-50 to-indigo-50 rounded-2xl flex items-center justify-center overflow-hidden">
        ...
        <div className="space-y-6">
          ...
          {reviewStats.count > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex">
                {[1, 2, 3, 4, 5].map(star => (
                  <Star key={star} size={16} className={star <= Math.round(reviewStats.average) ? 'text-amber-400 fill-amber-400' : 'text-slate-300'} />
                ))}
              </div>
              <span className="text-sm text-slate-600">{reviewStats.average.toFixed(1)} ({reviewStats.count} reviews)</span>
            </div>
          )}
          ...
          {product.stock > 0 && (
            <div className="flex items-center gap-4">
              <div className="flex items-center border border-slate-300 rounded-xl overflow-hidden">
                ...
                <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="px-4 py-2 hover:bg-slate-100 transition-colors">+</button>
              </div>
              <button onClick={handleAddToCart} className="btn btn-primary flex-1">
                <ShoppingCart size={18} />
                Add to Cart
              </button>
              <button onClick={handleToggleWishlist} className={`p-3 rounded-xl border transition-colors ${isWishlisted ? 'bg-red-50 border-red-200 text-red-500' : 'border-slate-300 text-slate-400 hover:text-red-400'}`}>
      ...
      <div className="card">
        ...
        <form onSubmit={handleSubmitReview} className="mb-8 p-4 bg-slate-50 rounded-xl space-y-4">
          <div>
            ...
            {reviews.map(review => (
              <div key={review.id} className="border-b border-slate-100 pb-4 last:border-0">
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map(star => (
                      <Star key={star} size={14} className={star <= review.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-300'} />
                    ))}
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {related.map(item => (
              <Link key={item.id} to={`/ecommerce/products/${item.id}`} className="card hover:shadow-lg transition-all hover:-translate-y-0.5 overflow-hidden">
                <div className="aspect-square bg-gradient-to-br from-slate-50 to-indigo-50 rounded-xl mb-3 flex items-center justify-center overflow-hidden">
                  {item.image_url ? (
