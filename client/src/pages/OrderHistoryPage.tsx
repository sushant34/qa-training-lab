...
const OrderHistoryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Order History</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">View your past orders and their details.</p>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="card text-center py-16">
          ...
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-1">No orders yet</h3>
          <p className="text-slate-500 dark:text-slate-400">Your completed orders will appear here.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(order.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                      ...
                      <div key={item.id} className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400">Qty: {item.quantity} × ${item.price.toFixed(2)}</p>
                          </div>
                        </div>
                        <span className="font-semibold text-slate-900 dark:text-slate-100">${(item.price * item.quantity).toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
